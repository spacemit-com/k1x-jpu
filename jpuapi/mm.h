...
#ifndef JPU_DRAM_VIRTUAL_BASE
#define JPU_DRAM_VIRTUAL_BASE 0x00000000
#endif

#if defined(JPU_SUBSYS)
typedef unsigned long long JpuVirtMemAddr;
#define _jdb_virt_to_phys(x, y) ((x) - (y))
#define _jdb_phys_to_virt(x, y) (JpuVirtMemAddr)((x) + (y))
#else
typedef unsigned long JpuVirtMemAddr;
#define _jdb_virt_to_phys(x, y) (x)
#define _jdb_phys_to_virt(x, y) (JpuVirtMemAddr)(x)
#endif
...
extern unsigned long jmem_alloc(jpeg_mm_t* mm, int size, unsigned long pid);
