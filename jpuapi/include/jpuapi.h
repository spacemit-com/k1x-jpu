...
struct codaJ12V_vcodec_parameters {
    ...
    int sample_bits;
    /** 1:decoder 0:encoder  when it is the decoder we so set luma stride = chroma stride YUV420**/
    int is_dec;
...
JpgRet		JPU_GetVersionInfo(JdiDeviceCtx devctx,
                               Uint32 *apiVersion,
                               Uint32 *hwRevision,
                               Uint32 *hwProductId);
...
JpgRet JPU_EncOpen(JdiDeviceCtx devctx, JpgEncHandle *pHandle, JpgEncOpenParam *pParam);
