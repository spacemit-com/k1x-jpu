...
JpgRet AsrJpuDecGetInitialInfo(void* handle,ImageBufferInfo *jpegImageBuffer,JpgDecInitialInfo *info) {
    ...
    pDecInfo->pBitStream = (BYTE *)mmap(NULL,jpegImageBuffer->dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,jpegImageBuffer->dmaBuffer.fd,0);
...
JpgRet AsrJpuDecStartOneFrame(void *handle,FrameBufferInfo* frameBuffer,ImageBufferInfo *jpegImageBuffer,JpgDecInitialInfo *info) {
    ...
    JPU_DMA_CFG cfg = jdi_config_mmu(pJpgInst->devctx,jpegImageBuffer->dmaBuffer.fd,frameBuffer->dmaBuffer.fd,frameBuffer->dmaBuffer.size,0);
    ...
    pDecInfo->streamBufEndAddr = cfg.intput_virt_addr + jpegImageBuffer->dmaBuffer.size;
    frameBuffer->dmaBuffer.viraddr = cfg.output_virt_addr;
    if (ret = JPU_DecSetRdPtrEx(handle, pDecInfo->streamWrPtr, TRUE) != JPG_RET_SUCCESS) {
        JLOG(ERR, "JPU_DecSetRdPtrEx failed Error code is 0x%x \n", ret ); 
    }
    // Register frame buffers requested by the decoder.
    if ((ret=JPU_DecRegisterFrameBuffer(handle, frameBuffer, 1, frameBuffer->stride)) != JPG_RET_SUCCESS) {
        JLOG(ERR, "JPU_DecRegisterFrameBuffer failed Error code is 0x%x \n", ret );
        return JPG_RET_FAILURE;
    }

    if ((ret = JPU_DecUpdateBitstreamBuffer(handle,jpegImageBuffer->imageSize? jpegImageBuffer->imageSize : jpegImageBuffer->dmaBuffer.size)) != JPG_RET_SUCCESS) {
    ...
    JLOG(INFO, "%02d %8d %8x %8x %10d %8x %8x %10d\n",
        instIdx, outputInfo.indexFrameDisplay, outputInfo.bytePosFrameStart, outputInfo.ecsPtr, outputInfo.consumedByte,
