...
int JpgDecHuffTabSetUp(JpgDecInfo *jpg, JdiDeviceCtx devctx, int instRegIndex)
{
    ...
    for(j=0; j<16; j++)
    {
        ...
        JpuWriteInstReg(devctx, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFF) << 16) | HuffData));	// 32-bit
    }
    ...
    for(j=0; j<16; j++)
    {
        ...
        temp = (HuffData & 0x8000) >> 15;
        temp = (temp << 15) | (temp << 14) | (temp << 13) | (temp << 12) | (temp << 11) | (temp << 10) | (temp << 9) | (temp << 8) | (temp << 7 ) | (temp << 6) | (temp <<5) | (temp<<4) | (temp<<3) | (temp<<2) | (temp<<1)| (temp) ;
        JpuWriteInstReg(devctx, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFF) << 16) | HuffData));
    }
    //DC Chroma
    for(j=0; j<16; j++)
    {
    ...
    for(j=0; j<16; j++)
    {
        ...
        JpuWriteInstReg(devctx, instRegIndex, MJPEG_HUFF_DATA_REG, (((temp & 0xFFFFFF) << 8) | HuffData));
    }
    //DC Chroma
    for(j=0; j<16; j++)
    {
...
int JpgDecHuffTabSetUp_12b(JpgDecInfo *jpg, JdiDeviceCtx devctx, int instRegIndex)
{
    int i, j;
    int HuffData;	// 16BITS
    int HuffLength;
...
void JpgDecGramSetup(JpgDecInfo * jpg, JdiDeviceCtx devctx, int instRegIndex)
{
    ...
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_CUR_POS_REG, dExtBitBufCurPos);
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_EXT_ADDR_REG, dExtBitBufBaseAddr + (dExtBitBufCurPos << 8));
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_INT_ADDR_REG, (dExtBitBufCurPos & 1) << 6);
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_DATA_CNT_REG, 256 / 4);	// 64 * 4 byte == 32 * 8 byte
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_COMMAND_REG, (jpg->streamEndian << 1) | 0);
    ...
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_CUR_POS_REG, dExtBitBufCurPos);	// next unit page pointer
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_BBC_CTRL_REG, (jpg->streamEndian<< 1) | 1);
    ...
    JpuWriteInstReg(devctx, instRegIndex, MJPEG_GBU_BBER_REG, ((256 / 4) * 2) - 1);
    ...
    if (jpg->pagePtr & 1) {
        ...
        JpuWriteInstReg(devctx, instRegIndex, MJPEG_GBU_BBIR_REG, 256 / 4);	// 64 * 4 byte == 32 * 8 byte
        JpuWriteInstReg(devctx, instRegIndex, MJPEG_GBU_BBHR_REG, 256 / 4);	// 64 * 4 byte == 32 * 8 byte
    }
...
static int wraparound_bistream_data(JpgDecInfo *jpg, JdiDeviceCtx devctx, int return_type)
{
#if 0
    BYTE *dst;
    BYTE *src;
...
JpgRet CheckJpgEncOpenParam(JpgEncOpenParam * pop, JPUCap* cap)
{
    ...
    JLOG(DBG, "----bitstreamBufferSize:%d picWidth:%d picHeight:%d intrEnableBit:%d---\n",pop->bitstreamBufferSize,picWidth,picHeight,pop->intrEnableBit);
...
int JpgEncLoadHuffTab(JpgInst *pJpgInst, int instRegIndex)
{
    ...
    for (j=0; j<4; j++)
    {
        ...
        for (i=0; i<256; i++)
        {
            ...
            JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_HUFF_DATA_REG, huffData);
        }
    }
...
int JpgEncLoadQMatTab(JpgInst *pJpgInst, int instRegIndex)
{
    ...
    for (comp=0; comp<3; comp++) {
        ...
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_QMAT_CTRL_REG, 0x3 + t);
        ...
        for (i=0; i<64; i++) {
            ...
            if (qprec) {
                JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(divisor & 0xFFFF));
                JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(quotient & 0xFFFFFFFF));
            } else
                JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_QMAT_DATA_REG, (int)(divisor<<20)|(int)(quotient & 0xFFFFF));
