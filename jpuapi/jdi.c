...
#define JDI_INSTANCE_POOL_TOTAL_SIZE    (JDI_INSTANCE_POOL_SIZE + sizeof(pthread_mutex_t) * JDI_NUM_LOCK_HANDLES)
...
JdiDeviceCtx jdi_init(int dev_id)
{
    ...
    jdi->jdb_register.virt_addr = (unsigned long)mmap/*64*/(NULL, jdi->jdb_register.size, PROT_READ | PROT_WRITE, MAP_SHARED, jdi->jpu_fd, /*(off64_t)*/(jdi->jdb_register.phys_addr));
...
int jdi_release(JdiDeviceCtx devctx)
{
    ...
    if (jdi->jdb_register.virt_addr) {
        if (munmap((void *)jdi->jdb_register.virt_addr, jdi->jdb_register.size) < 0) {
...
jpu_instance_pool_t *jdi_get_instance_pool(JdiDeviceCtx devctx)
{
    ...
    if (!jdi->pjip) {
        ...
        JLOG(DBG, "[JDI] instance pool physaddr=%p, virtaddr=%p, base=%p, size=%d\n", jdb.phys_addr, jdb.virt_addr, jdb.base, jdb.size);
    }
...
void jdi_write_register(JdiDeviceCtx devctx, unsigned long addr, unsigned int data)
{
    jdi_info_t *jdi = (jdi_info_t *)devctx;
...
int jdi_wait_inst_ctrl_busy(JdiDeviceCtx devctx, int timeout, unsigned int addr_flag_reg, unsigned int flag)
{
...
int jdi_wait_interrupt(JdiDeviceCtx devctx, int timeout, unsigned int addr_int_reason, unsigned long instIdx)
{
    jdi_info_t *jdi = (jdi_info_t*)devctx;
    int intr_reason = 0;
    int ret;
...
JPU_DMA_CFG jdi_config_mmu(JdiDeviceCtx devctx,int input_buffer_fd,int output_buffer_fd,unsigned int data_size,unsigned int append_size)
{
    jdi_info_t *jdi = (jdi_info_t*)devctx;
    int ret;
    JPU_DMA_CFG cfg;
