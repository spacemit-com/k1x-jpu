...
JpgRet AsrJpuEncOpen(void** handle, EncOpenParam* param) {
    ...
    JLOG(DBG, "JPU Version API_VERSION=0x%x, HW_REVISION=%d, HW_PRODUCT_ID=0x%x\n", apiVersion, hwRevision, hwProductId);
...
static int JpuEncQualityFactor(JpgEncInfo *encInfo,Uint32 quality, BOOL useStdTable)
{
    Uint32 scaleFactor;
    Uint32 i;
    Uint32 temp;
    ...
    */
    ...
    for (i=0; i<64; i++) {
        pLumaQTable[i] = (useStdTable == TRUE) ? std_luminance_quant_tbl[i] : (Int32)encInfo->pQMatTab[DC_TABLE_INDEX0][i];
    }
    for (i=0; i<64; i++) {
        pChromaQTable[i] = (useStdTable == TRUE) ? std_chrominance_quant_tbl[i] : (Int32)encInfo->pQMatTab[AC_TABLE_INDEX0][i];
    }
    ...
    */
    ...
    memcpy((void*)encInfo->pQMatTab[DC_TABLE_INDEX0], (void*)lumaQ2, 64*qSize);

    memset((void*)encInfo->pQMatTab[AC_TABLE_INDEX0], 0x00, 64*qSize);
    memcpy((void*)encInfo->pQMatTab[AC_TABLE_INDEX0], (void*)chromaBQ2, 64*qSize);
#endif

    qTableSize = 64*qSize;
    memcpy((void*)encInfo->pQMatTab[DC_TABLE_INDEX1], (void*)encInfo->pQMatTab[DC_TABLE_INDEX0], qTableSize);
    memcpy((void*)encInfo->pQMatTab[AC_TABLE_INDEX1], (void*)encInfo->pQMatTab[AC_TABLE_INDEX0], qTableSize);
...
JpgRet AsrJpuEncSetParam(void *handle,Uint32 parameterIndex, void *value) {
    ...
    switch (parameterIndex)
    {
        ...
        case JPU_ROTATION:
            ...
            JPU_EncHandleRotaion(&pEncHandler->JpgInfo->encInfo,pEncHandler->JpgInfo->encInfo.rotationIndex);
            break;
        case JPU_MIRROR:
            pEncHandler->JpgInfo->encInfo.mirrorIndex = *(Uint32 *)value;
            break;
        case JPU_QUALITY:
            JpuEncQualityFactor(&pEncHandler->JpgInfo->encInfo,*(Uint32 *)value,FALSE);
            break;
        case JPU_HUFFMAN_TAB:
            JPUEncGetHuffTable((char*)(value),mjpgParam, encInfo->jpg12bit);
            if (encInfo->jpg12bit) {
                for (i=0; i<8; i++) {
                    memcpy(pEncHandler->JpgInfo->encInfo.pHuffVal[i],mjpgParam->huffVal[i],256);
                    memcpy(pEncHandler->JpgInfo->encInfo.pHuffBits[i],mjpgParam->huffBits[i],256);
                }
            } else {
                for (i=0; i<4; i++) {
                    memcpy(pEncHandler->JpgInfo->encInfo.pHuffVal[i],mjpgParam->huffVal[i],256);
        ...
        case JPU_QUANT_TAB:
            ...
            for (i=0; i<4; i++) {
                memcpy(pEncHandler->JpgInfo->encInfo.pQMatTab[i],mjpgParam->qMatTab[i],64*sizeof(short));
            }
            break;
...
JpgRet AsrJpuEncStartOneFrame(void *handle,FrameBufferInfo* frameBuffer,ImageBufferInfo *jpegImageBuffer) {
    ...
    headerParamSet.disableAPPMarker = JpgEncHandle->JpgInfo->encInfo.disableAPPMarker;
    headerParamSet.enableSofStuffing = JpgEncHandle->JpgInfo->encInfo.stuffByteEnable;
    headerParamSet.disableSOIMarker = JpgEncHandle->JpgInfo->encInfo.disableSOIMarker;
    headerParamSet.headerMode = ENC_HEADER_MODE_NORMAL;            //Encoder header disable/enable control. Annex:A 1.2.3 item 13
    headerParamSet.quantMode = JPG_TBL_NORMAL; //JPG_TBL_MERGE    // Merge quantization table. Annex:A 1.2.3 item 7
    headerParamSet.huffMode  = JPG_TBL_NORMAL; // JPG_TBL_MERGE    //Merge huffman table. Annex:A 1.2.3 item 
    //headerParamSet.pParaSet = (BYTE *)mmap(NULL,jpegImageBuffer->dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,jpegImageBuffer->dmaBuffer.fd,0);
    inputDmaBufVir = (BYTE *)mmap(NULL,jpegImageBuffer->dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,jpegImageBuffer->dmaBuffer.fd,0);
    headerParamSet.pParaSet = inputDmaBufVir + jpegImageBuffer->dataOffset;
    headerParamSet.size = jpegImageBuffer->dmaBuffer.size - jpegImageBuffer->dataOffset;
    ...
    JpgEncHandle->JpgInfo->encInfo.streamBodyOffset = imageHeaderSize + jpegImageBuffer->dataOffset;
    JpgEncHandle->JpgInfo->encInfo.streamSize = jpegImageBuffer->dmaBuffer.size - JpgEncHandle->JpgInfo->encInfo.streamBodyOffset;
    ...
    if ((ret=JPU_EncGetOutputInfo(JpgEncHandle, &outputInfo)) != JPG_RET_SUCCESS) {
        JLOG(ERR, "JPU_EncGetOutputInfo failed Error code is 0x%x \n", ret );
    }
    imageDataPtr = headerParamSet.pParaSet+outputInfo.bitstreamSize+imageHeaderSize;
    while (outputInfo.bitstreamSize) {
        if (*(imageDataPtr)==0xff) {
            outputInfo.bitstreamSize--;
            JLOG(DBG, "%s:find stuff byte :%p value:%x\n",__func__,imageDataPtr,*imageDataPtr);
