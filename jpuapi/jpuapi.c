...
#define show_register(reg)                                                                              \
    do {                                                                                                \
        JLOG(ERR, "device %p: register %s, %04x@%08x\n", handle->devctx, #reg, reg,                    \
                JpuReadInstReg(handle->devctx, instRegIndex, reg));                                     \
    } while (0)
...
Int32   JPU_WaitInterrupt(JpgHandle handle, int timeout)
{
    ...
    reason = jdi_wait_interrupt(pJpgInst->devctx, timeout, instPicStatusRegAddr, instRegIndex);
    if (reason == -1) {
...
JpgRet JPU_GetVersionInfo(JdiDeviceCtx devctx, Uint32 *apiVersion, Uint32 *hwRevision, Uint32 *hwProductId)
{
...
JpgRet JPU_DecOpen(JdiDeviceCtx devctx, JpgDecHandle * pHandle, JpgDecOpenParam * pop)
{
    JpgInst*    pJpgInst;
    JpgDecInfo* pDecInfo;
    JpgRet      ret;
...
JpgRet JPU_DecGetInitialInfo(JpgDecHandle handle, JpgDecInitialInfo * info)
{
    ...
    pJpgInst = handle;
    pDecInfo = &pJpgInst->JpgInfo->decInfo;
    if (JpegDecodeHeader(pDecInfo, pJpgInst->devctx) <= 0)
        return JPG_RET_FAILURE;
...
JpgRet JPU_DecRegisterFrameBuffer(JpgDecHandle handle, FrameBufferInfo * bufArray, int num, int stride)
{
    JpgInst * pJpgInst;
    JpgDecInfo * pDecInfo;
    JpgRet ret;
                                 ...
                                 int * size)
{
    ...
    if (GetJpgPendingInstEx(pJpgInst->devctx, pJpgInst->instIndex) == pJpgInst) {
        rdPtr = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_RD_PTR_REG);
    }
    else {
...
JpgRet JPU_DecUpdateBitstreamBuffer(JpgDecHandle handle, int size)
{
    ...
    if (size == 0)
    {
        ...
        if ((wrPtr-pDecInfo->streamBufStartAddr) % 256)
            val = val + 1;
        //if (GetJpgPendingInstEx(pJpgInst->devctx, pJpgInst->instIndex) == pJpgInst) {
            JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_STRM_CTRL_REG, (1UL << 31 | val));
        //}
    ...
    if (GetJpgPendingInstEx(pJpgInst->devctx, pJpgInst->instIndex) == pJpgInst) {
        ...
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_WR_PTR_REG, wrPtr);
        if (wrPtr == pDecInfo->streamBufStartAddr) {
            JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_END_ADDR_REG, pDecInfo->streamBufEndAddr);
        }
        else {
            JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_END_ADDR_REG, wrPtr);
        }
    }
    else {
        rdPtr = pDecInfo->streamRdPtr;
...
JpgRet JPU_DecStartOneFrame(JpgDecHandle handle, JpgDecParam *param)
{
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_INTR_MASK_REG, ((~pDecInfo->intrEnableBit) & 0x3ff));
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_WR_PTR_REG, pDecInfo->streamWrPtr);
    if (pDecInfo->streamWrPtr == pDecInfo->streamBufStartAddr) {
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_END_ADDR_REG, pDecInfo->streamBufEndAddr);
    }
    else {
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_END_ADDR_REG, JPU_CEIL(256, pDecInfo->streamWrPtr));
    }

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_BAS_ADDR_REG, pDecInfo->streamBufStartAddr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_TCNT_REG, 0);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, (MJPEG_GBU_TCNT_REG+4), 0);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_ERRMB_REG, 0);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_CTRL_REG,pDecInfo->huffAcIdx << 10 | pDecInfo->huffDcIdx << 7 | pDecInfo->userHuffTab << 6 |
                    (JPU_CHECK_WRITE_RESPONSE_BVALID_SIGNAL << 2) | 0);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_SIZE_REG, (pDecInfo->alignedWidth << 16) | pDecInfo->alignedHeight);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_OP_INFO_REG,  pDecInfo->busReqNum);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_MCU_INFO_REG, (pDecInfo->mcuBlockNum&0x0f) << 16 | (pDecInfo->compNum&0x07) << 12    |
    ...
    //packedFormat:5 => 8
    //packedFormat:6 => 9
    val = (pDecInfo->frameEndian << 6) | ((pDecInfo->chromaInterleave==0)?0:(pDecInfo->chromaInterleave==1)?2:3);
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_RST_INTVAL_REG, pDecInfo->rstIntval);
    ...
    if (pDecInfo->streamEndflag == 1) {
        val = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_STRM_CTRL_REG);
        if ((val & (1UL << 31)) == 0 ) {
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_RST_INDEX_REG, 0);	// RST index at the beginning.
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_FF_RPTR_REG, pDecInfo->bitPtr);
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_ROT_INFO_REG, (ppuEnable<<4) | (pDecInfo->mirrorIndex<<2) | pDecInfo->rotationIndex);

    val = (pDecInfo->frameIdx % pDecInfo->numFrameBuffers);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE00_REG, frame_virt_addr + pDecInfo->frameBufPool[val].yOffset);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE01_REG, frame_virt_addr + pDecInfo->frameBufPool[val].uOffset);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE02_REG, frame_virt_addr + pDecInfo->frameBufPool[val].vOffset);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_YSTRIDE_REG, pDecInfo->stride);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_CSTRIDE_REG, pDecInfo->stride_c);

    if (pDecInfo->roiEnable) {
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CLP_INFO_REG, 1);
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CLP_BASE_REG, pDecInfo->roiOffsetX << 16 | pDecInfo->roiOffsetY);	// pixel unit
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CLP_SIZE_REG, (pDecInfo->roiFrameWidth << 16) | pDecInfo->roiFrameHeight); // pixel Unit
    }
    else {
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CLP_INFO_REG, 0);
    }
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_START_REG, (1<<JPG_START_PIC));
    pDecInfo->decIdx++;
...
JpgRet JPU_DecGetOutputInfo(JpgDecHandle handle, JpgDecOutputInfo *info)
{
    ...
    info->consumedByte = (JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_TCNT_REG))/8;
    pDecInfo->streamRdPtr = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_RD_PTR_REG);
    ...
    info->wrPtr  = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_WR_PTR_REG);
...
//   intStatus = JpuReadInstReg(instRegIndex, MJPEG_PIC_STATUS_REG);
    if (intStatus & (1<<INT_JPU_DONE)) {
        ...
        info->numOfErrMBs = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_ERRMB_REG);
        info->decodingSuccess = 0;

    }
    else if (intStatus & (1<<INT_JPU_OVERFLOW)) {
        info->decodeState      = DECODE_STATE_SLICE_DONE;
        info->decodedSliceYPos = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_SLICE_POS_REG);
        pDecInfo->decSlicePosY = info->decodedSliceYPos;
    }
    info->frameCycle = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CYCLE_INFO_REG);
...
JpgRet JPU_EncOpenParamDefault(JpgEncOpenParam *pEncOP)
{
    ...
    pEncOP->intrEnableBit = (1<<INT_JPU_DONE) | (1<<INT_JPU_ERROR) | (1<<INT_JPU_BIT_BUF_FULL);
    pEncOP->mirror = 0;
    pEncOP->rotation = 0;
...
JpgRet JPU_EncHandleRotaion(JpgEncInfo* pEncInfo,Uint32 rotationIndex) {
    ...
    if (pEncInfo->format == FORMAT_420) {
        ...
        Uint32 picHeight = (rotationIndex == 1 || rotationIndex == 3) ? pEncInfo->picWidth : pEncInfo->picHeight;
...
JpgRet JPU_EncOpen(JdiDeviceCtx devctx, JpgEncHandle *pHandle, JpgEncOpenParam* pop)
{
    JpgInst*    pJpgInst;
    JpgEncInfo* pEncInfo;
    JpgRet      ret;
...
JpgRet JPU_EncStartOneFrame(JpgEncHandle handle, JpgEncParam * param)
{
     ...
     *    @@@@@@@@@@@        @@@@@@@@@@@         ##########@          @##########          *
     *                                                                                     *
     *  after mirror or rotation  2/3/4 should append dma pages to mmu for the bug 128642  *
     * ************************************************************************************/
    dataSize = pEncInfo->alignedWidth * pEncInfo->alignedHeight * 3/2;
    if ((pEncInfo->mirrorIndex == 1 && pEncInfo->rotationIndex == 1) ||(pEncInfo->mirrorIndex == 3 && pEncInfo->rotationIndex == 1)
        || (pEncInfo->mirrorIndex == 0 && pEncInfo->rotationIndex == 3) ||(pEncInfo->mirrorIndex == 1 && pEncInfo->rotationIndex == 0)
        || (pEncInfo->mirrorIndex == 2 && pEncInfo->rotationIndex == 2)) {
        appendingSize = (pEncInfo->alignedHeight - pEncInfo->srcHeight) * pEncInfo->alignedWidth;
    } else if ((pEncInfo->mirrorIndex == 1 && pEncInfo->rotationIndex == 2) || (pEncInfo->mirrorIndex == 2 && pEncInfo->rotationIndex == 0)
        ||(pEncInfo->mirrorIndex == 0 && pEncInfo->rotationIndex == 1) || (pEncInfo->mirrorIndex == 3 && pEncInfo->rotationIndex == 3)) {
        appendingSize = (pEncInfo->alignedWidth - pEncInfo->srcWidth);
    } else if ((pEncInfo->mirrorIndex == 0 && pEncInfo->rotationIndex == 2) || (pEncInfo->mirrorIndex == 1 && pEncInfo->rotationIndex == 1)
        || (pEncInfo->mirrorIndex == 2 && pEncInfo->rotationIndex == 3) || (pEncInfo->mirrorIndex == 3 && pEncInfo->rotationIndex == 0)) {
        appendingSize = (pEncInfo->alignedHeight - pEncInfo->srcHeight) * pEncInfo->alignedWidth;
        appendingSize += (pEncInfo->alignedWidth - pEncInfo->srcWidth);
    }

    //JLOG(INFO, "######## input fd:%d output fd:%d slice mode:%d slice y:%d  algin height:%d ######### \n",param->sourceFrame->dmaBuffer.fd,pEncInfo->streamFd,pJpgInst->sliceInstMode,pEncInfo->encSlicePosY,pEncInfo->alignedHeight);
    JPU_DMA_CFG cfg = jdi_config_mmu(pJpgInst->devctx,param->sourceFrame->dmaBuffer.fd,pEncInfo->streamFd,dataSize,appendingSize);
    ...
    pEncInfo->streamBufStartAddr = cfg.output_virt_addr + pEncInfo->streamBodyOffset;
    pEncInfo->streamBufEndAddr   = cfg.output_virt_addr + pEncInfo->streamSize + pEncInfo->streamBodyOffset;
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_INTR_MASK_REG, ((~pEncInfo->intrEnableBit) & 0x7ff));
    //JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_SLICE_INFO_REG, pEncInfo->sliceHeight);
    //JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_SLICE_DPB_POS_REG, pEncInfo->picHeight); // assume that the all of source buffer is available
    //JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_SLICE_POS_REG, pEncInfo->encSlicePosY);
    val = (0 << 16) | (pEncInfo->encSlicePosY / pEncInfo->mcuHeight);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_SETMB_REG,0);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_CLP_INFO_REG, 0);	//off ROI enable due to not supported feature for encoder.

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_BAS_ADDR_REG, pEncInfo->streamBufStartAddr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_END_ADDR_REG, pEncInfo->streamBufEndAddr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_WR_PTR_REG, pEncInfo->streamWrPtr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_RD_PTR_REG, pEncInfo->streamRdPtr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_CUR_POS_REG, 0);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_DATA_CNT_REG, JPU_GBU_SIZE / 4);	// 64 * 4 byte == 32 * 8 byte
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_EXT_ADDR_REG, pEncInfo->streamBufStartAddr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_INT_ADDR_REG, 0);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_BAS_ADDR_REG, pEncInfo->streamWrPtr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_EXT_ADDR_REG, pEncInfo->streamRdPtr);
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_BBER_REG, ((JPU_GBU_SIZE / 4) * 2) - 1);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_BBIR_REG, JPU_GBU_SIZE / 4);	// 64 * 4 byte == 32 * 8 byte
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_BBHR_REG, JPU_GBU_SIZE / 4);	// 64 * 4 byte == 32 * 8 byte

//#define DEFAULT_TDI_TAI_DATA 0x055
//    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_CTRL_REG, (pEncInfo->jpg12bit<<31) | (pEncInfo->q_prec0<<30) | (pEncInfo->q_prec1<<29) | (pEncInfo->tiledModeEnable<<19) |
//                                                     (DEFAULT_TDI_TAI_DATA<<7) | 0x18 | (1<<6) | (JPU_CHECK_WRITE_RESPONSE_BVALID_SIGNAL<<2));
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_CTRL_REG,1<<6|1<<4|1<<3);
    ...
    //JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_CONFIG_REG, val);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_CONFIG_REG, 2);//yuv NV12

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_RST_INTVAL_REG, pEncInfo->rstIntval);
    if (pEncInfo->encSlicePosY == 0) {
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_RST_INDEX_REG, 0);// RST index from 0.
        JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_STRM_CTRL_REG, 0);// clear BBC ctrl status.
    }

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_CTRL_REG, (pEncInfo->streamEndian << 1) | 1);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_OP_INFO_REG, pEncInfo->busReqNum);
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_PIC_SIZE_REG, pEncInfo->alignedWidth<<16 | pEncInfo->alignedHeight);
    ...
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_ROT_INFO_REG, rotMirEnable | rotMirMode);
    JLOG(DBG, "##mcuBlockNum %x compNum :%x  compInfo0:%x compInfo1:%x  compInfo2:%x rotMirMode:%d  ### \n",pEncInfo->mcuBlockNum,pEncInfo->compNum,pEncInfo->compInfo[0],pEncInfo->compInfo[1],pEncInfo->compInfo[2],rotMirMode);

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_MCU_INFO_REG, (pEncInfo->mcuBlockNum&0x0f) << 16 | (pEncInfo->compNum&0x07) << 12    |
                    (pEncInfo->compInfo[0]&0x3f) << 8  | (pEncInfo->compInfo[1]&0x0f) << 4 |
                    (pEncInfo->compInfo[2]&0x0f));

    //JpgEncGbuResetReg
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_GBU_CTRL_REG, pEncInfo->stuffByteEnable<<3);     // stuffing "FF" data where frame end

    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE00_REG,  cfg.intput_virt_addr);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE01_REG,  cfg.intput_virt_addr+pBasFrame->uOffset + appendingSize/2);//appendingSize will not zero only when case 2/3/4 above
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_BASE02_REG,  cfg.intput_virt_addr+pBasFrame->vOffset + appendingSize/2);//appendingSize will not zero only when case 2/3/4 above
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_YSTRIDE_REG, pBasFrame->stride);
    JpuWriteInstReg(pJpgInst->devctx, instRegIndex, MJPEG_DPB_CSTRIDE_REG, pBasFrame->stride);
    JLOG(DBG, "### y:%x uv:%x  ystride:%d cstride:%d ### \n",cfg.intput_virt_addr,cfg.intput_virt_addr+pBasFrame->stride*JPU_CEIL(8,pEncInfo->picHeight),pBasFrame->stride,pBasFrame->strideC);
...
//    intReason = JpuReadInstReg(instRegIndex, MJPEG_PIC_STATUS_REG);
    ...
    info->encodedSliceYPos = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_SLICE_POS_REG);
    pEncInfo->encSlicePosY = info->encodedSliceYPos;
    if (intReason & (1<<INT_JPU_DONE))
        pEncInfo->encSlicePosY = 0;

    PhysicalAddress streamWrPtr = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_WR_PTR_REG);
    PhysicalAddress streamRdPtr = JpuReadInstReg(pJpgInst->devctx, instRegIndex, MJPEG_BBC_RD_PTR_REG);
