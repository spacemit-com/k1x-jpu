...
JpuThread JpuThread_Create(JpuThreadRunner runner, void *arg) {
  ...
  thread = CreateThread(NULL, 20 * 1024 * 1024, (LPTHREAD_START_ROUTINE)runner,
                        arg, 0, /* Creation Flag */
                        &threadId);
...
BOOL JpuThread_Join(JpuThread thread) {
  ...
  if (hJpuThread == NULL) return FALSE;
...
void MSleep(Uint32 ms) { Sleep(ms); }

/*
 * returns current time in ms unit
 */
Uint64 GetCurrentTimeInMs(void) { return 0; }
...
JpuThread JpuThread_Create(JpuThreadRunner runner, void* arg) {
  ...
  if ((ret = pthread_create((pthread_t*)thread, NULL,
                            (PTHREAD_START_ROUTINE)runner, arg)) != 0) {
    free(thread);
...
void MSleep(Uint32 ms) { usleep(ms * 1000); }
...
void MSleep(Uint32 ms) { UNREFERENCED_PARAMETER(ms); }
