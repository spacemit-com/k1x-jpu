...
static int StoreYuvImageBurstFormat_V20(BufferAllocator*  bufferAllocator,int chromaStride, Uint8 * dst, int picWidth, int picHeight, Uint32 bitDepth, void* addrY, void* addrCb, void* addrCr, int stride, FrameFormat format,CbCrInterLeave intlv, PackedFormat packed);
...
)
{
    ...
    data = mmap(NULL,fb->dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,fb->dmaBuffer.fd,0);
    frameSize = StoreYuvImageBurstFormat_V20(bufferAllocator,fb->strideC, pYuv, picWidth, picHeight, bitDepth,
                data + fb->yOffset,
                data + fb->uOffset,
                data + fb->vOffset,
                fb->stride,
...
int StoreYuvImageBurstFormat_V20(BufferAllocator*  bufferAllocator,int chromaStride, Uint8 * dst, int picWidth, int picHeight, Uint32 bitDepth,
                                 void* addrY, void* addrCb, void* addrCr, int stride, FrameFormat format, CbCrInterLeave interLeave, PackedFormat packed)
{
    int size;
    int y, nY = 0, nCb, nCr;
    void *  addr;
...
FrameBufferInfo* AllocateFrameBuffer(BufferAllocator*  bufferAllocator,Uint32 instIdx, FrameFormat subsample, CbCrInterLeave cbcrIntlv, PackedFormat packed, 
                         Uint32 rotation, BOOL scalerOn, Uint32 width, Uint32 height, Uint32 bitDepth)
{
    ...
    if (fbChromaSize) {
        ...
        frameBufferInfo->vOffset = (cbcrIntlv == CBCR_SEPARATED) ? fbLumaSize+ fbChromaSize: 0;
    }
    frameBufferInfo->dmaBuffer.fd = DmabufHeapAllocSystem(bufferAllocator,true,frameBufferInfo->dmaBuffer.size,0,0);
...
void FreeFrameBuffer(FrameBufferInfo *frameBufferInfo)
{
    ...
    if (frameBufferInfo->dmaBuffer.fd >= 0)
        close(frameBufferInfo->dmaBuffer.fd);
