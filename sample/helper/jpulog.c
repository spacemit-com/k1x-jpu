...
void SetLogColor(int level, int color) { log_colors[level] = color; }

int GetLogColor(int level) { return log_colors[level]; }

void SetLogDecor(int decor) { log_decor = decor; }

int GetLogDecor() { return log_decor; }
...
#ifdef WIN32
  if (log_decor & LOG_HAS_COLOR) term_set_color(log_colors[level]);
#endif
...
#ifdef WIN32
  if (log_decor & LOG_HAS_COLOR) term_restore_color();
#endif
}  // lint !e438
...
static void term_set_color(int color) {
  ...
  if (color & TERM_COLOR_R) attr |= FOREGROUND_RED;
  if (color & TERM_COLOR_G) attr |= FOREGROUND_GREEN;
  if (color & TERM_COLOR_B) attr |= FOREGROUND_BLUE;
  if (color & TERM_COLOR_BRIGHT) attr |= FOREGROUND_INTENSITY;

  SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), attr);
}

static void term_restore_color(void) { term_set_color(log_colors[4]); }
#endif
...
void timer_init() {
  ...
  if (QueryPerformanceFrequency(&frequency_)) {
    ...
    // frequency_.QuadPart, frequency_.LowPart);
    timer_frequency_ =
        (double)((long double)((long)(frequency_.HighPart) >> 32) +
                 frequency_.LowPart);  // lint !e572
  } else
...
#else
#if defined(_MSC_VER)
  if (timer_frequency_ == 0) return 0;

  elapsed = (double)((long double)(counter_.QuadPart - initial_.QuadPart) /
                     (long double)frequency_.QuadPart);
#endif
#endif
  return elapsed;
}

int timer_is_valid() { return timer_frequency_ != 0; }

double timer_frequency() { return timer_frequency_; }
