...
static void CopyYuvData(JdiDeviceCtx devctx, void *fbAddr, Uint32 fbStride, Uint8* data, Uint32 dataStride, Uint32 dataHeight, Uint32 endian)
{
    Uint8* addr  = fbAddr;
    Uint8*          pData = data;
    Uint32          i;
...
)
{
    ...
    Uint32 framebufWidth  = (fb->format == FORMAT_420 || fb->format == FORMAT_422) ? JPU_CEIL(16, attr.width)  : JPU_CEIL(8, attr.width);
    Uint32 framebufHeight = (fb->format == FORMAT_420 || fb->format == FORMAT_440) ? JPU_CEIL(16, attr.height) : JPU_CEIL(8, attr.height);
    ...
    pY = ctx->pYuv;
    pFrameBuffer = (Uint8*)mmap(NULL,fbLumaSize+fbChromaSize,PROT_READ|PROT_WRITE,MAP_SHARED,fb->dmaBuffer.fd,0);
    switch (attr.format) {
    case FORMAT_420:
        if (attr.packedFormat == PACKED_FORMAT_NONE) {
            CopyYuvData(ctx->devctx,pFrameBuffer, fb->stride, pY, ctx->lumaLineWidth, ctx->lumaHeight, endian);
            pU = pY + ctx->lumaSize;
            if (attr.chromaInterleaved == CBCR_SEPARATED) {
                fbLumaHeight = framebufHeight;
                fbChromaHeight = fbLumaHeight/2;
                fbLumaSize = fbLumaStride*fbLumaHeight;
                CopyYuvData(ctx->devctx, pFrameBuffer+fbLumaSize, fb->strideC, pU, ctx->chromaLineWidth, ctx->chromaHeight, endian);
                pV = pU + ctx->chromaSize;
                CopyYuvData(ctx->devctx, pFrameBuffer+fbLumaSize+fbLumaSize/4, fb->strideC, pV, ctx->chromaLineWidth, ctx->chromaHeight, endian);
            }
            else {
                fbLumaHeight = framebufHeight;
                fbChromaHeight = fbLumaHeight/2;
            ...
            CopyYuvData(ctx->devctx, pFrameBuffer+fbLumaSize, fb->stride, pY, ctx->lumaLineWidth, ctx->lumaHeight, endian);
        }
        break;
    default:
...
)
{
    ...
    pFrameBufferVir = (Uint8*)mmap(NULL,fb->dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,fb->dmaBuffer.fd,0);
    ...
    case FORMAT_444:
        if (attr.packedFormat == PACKED_FORMAT_NONE) {
            ...
            memcpy(pFrameBufferVir+fb->yOffset+lumaOffset, pY+lumaOffset, ctx->lumaLineWidth);
            if (attr.format != FORMAT_444) {
                memcpy(pFrameBufferVir+fb->yOffset+lumaOffset+ctx->lumaLineWidth, pY+lumaOffset+ctx->lumaLineWidth, ctx->lumaLineWidth);
            }
            pU = pY + ctx->lumaSize;
            if (attr.chromaInterleaved == TRUE) {
                chromaOffset = currentChromaRow * ctx->chromaLineWidth * 2;
                memcpy(pFrameBufferVir+fb->uOffset+chromaOffset, pU+chromaOffset, 2*ctx->chromaLineWidth);
            }
            else {
                chromaOffset = currentChromaRow * ctx->chromaLineWidth;
                memcpy(pFrameBufferVir+fb->uOffset+chromaOffset, pU+chromaOffset, ctx->chromaLineWidth);
                pV = pU + ctx->chromaSize;
                memcpy(pFrameBufferVir+fb->vOffset+chromaOffset, pV+chromaOffset, ctx->chromaLineWidth);
            }
...
)
{
    if (feeder->sliceHeight > 0) {
        return LoadSliceFromFile((IYuvContext*)feeder->impl->context, fb, feeder->attr, endian,bufferAllocator);
    }
    else {
        return LoadFrameFromFile((IYuvContext*)feeder->impl->context, fb, feeder->attr, endian,bufferAllocator);
    }
}
...
)
{
    ...
    if (mode == YUV_FEEDER_MODE_THREAD) {
        ...
        feeder->fbQueue  = Queue_Create_With_Lock(FB_QUEUE_SIZE, sizeof(FrameBufferInfo));
    }
