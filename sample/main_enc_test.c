...
static void CalcSliceHeight(EncOpenParam* encOP, Uint32 sliceHeight,Uint32 rotation,int *restartInterval,BOOL *sliceInstMode)
{
    Uint32 mSliceHeight;
...
)
{
    ...
    JLOG(INFO, "--cfg-dir=DIR           folder that has encode parameters default: %s\n", CFG_DIR);
...
)
{
    ...
    while (loop_count) {
        ...
        jpegImageBuffer.dmaBuffer.fd = DmabufHeapAllocSystem(bufferAllocator,true,jpegImageBuffer.dmaBuffer.size,0,0);
        if (jpegImageBuffer.dmaBuffer.fd < 0) {
            JLOG(ERR, "#### DmabufHeapAllocSystem alloc stream fd:%d size:%d is invaild ####\n",jpegImageBuffer.dmaBuffer.fd,jpegImageBuffer.dmaBuffer.size);
            goto ERR_ENC;
        }

        //jpegImageVirtAddr = mmap(NULL,jpegImageBuffer.dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,jpegImageBuffer.dmaBuffer.fd,0);
        ...
        frameBuffer = AllocateFrameBuffer(bufferAllocator,0, encOP.sourceFormat, encConfig.chromaInterleave, encConfig.packedFormat, encConfig.rotation, 0, framebufWidth, framebufHeight, bitDepth);
        if (frameBuffer == NULL) {
        ...
        if ((yuvFeeder=YuvFeeder_Create(YUV_FEEDER_MODE_NORMAL, yuvPath, sourceAttr, JDI_LITTLE_ENDIAN, NULL, devctx)) == NULL) {
            goto ERR_ENC;
        }
        ...
        if (profiling) {
            ...
            total_time += (end_time.tv_sec - start_time.tv_sec) * 1000.f + (end_time.tv_usec - start_time.tv_usec) / 1000.f;      
        }
        jpegImageVirtAddr = mmap(NULL,jpegImageBuffer.dmaBuffer.size,PROT_READ|PROT_WRITE,MAP_SHARED,jpegImageBuffer.dmaBuffer.fd,0);
        if (FALSE == BitstreamWriter_Act(writer, jpegImageVirtAddr+0, esSize, FALSE)) {
            goto ERR_ENC;
        }
...
ERR_ENC:
        ...
        if (handle != NULL)
            AsrJpuEncClose(handle);
        loop_count--;
    }
    if (profiling)
        JLOG(INFO, "-----frameIdx:%d ,performance:%.2f fps. ------\n",frameIdx,frameIdx * 1000 / total_time);
